...
sys.path.append("/home/alex/infill/SparseConvNet")
...
class SparseInfill(nn.Module):
    def __init__(self, inputshape, reps, filter_size, nin_features, nout_features, nplanes, show_sizes, downsample=[2,2]):
        ...
        self.sparseModel = scn.Sequential().add(
           ...
           scn.SubmanifoldConvolution(self._dimension, 1, self._nin_features, self._filter_size, False)).add(
           scn.UNet(self._dimension, self._reps, self._nplanes, residual_blocks=True, downsample=downsample)).add(
        ...
        self.conv1 = scn.SubmanifoldConvolution(self._dimension, 1, self._nin_features, self._filter_size, False)
        self.unet = scn.UNet(self._dimension, self._reps, self._nplanes, residual_blocks=True, downsample=downsample)
        self.batchnorm = scn.BatchNormReLU(self._nin_features)
        self.output = scn.OutputLayer(self._dimension)
        self.conv2 = scn.SubmanifoldConvolution(self._dimension, self._nin_features, 1, self._filter_size, False)
